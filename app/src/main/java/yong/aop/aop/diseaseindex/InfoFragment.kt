    ...
    private fun readData() {
        ...
        database.child(disarray[i]).get().addOnSuccessListener {
            if(it.child(starray[j]+"-5").value.toString()=="null" && it.child(starray[j]+"-4").value.toString()!="null") {
                ...
                binding!!.tvInfodiss1.text=it.child(starray[j]).value.toString()
            }else{
            }
